...
*          50 <= ISPEC <= 56:
*               PxHGEQZ or one of its subroutines,
*               see PIPARMQ for detailed explanation
*          80 <= ISPEC <= 85:
*               Parameters for PBxGTSEN, as follows:
*               =80: maximum number of concurrent computational windows;
*               =81: number of eigenvalues/bulges in each window;
*               =82: computational window size;
*               =83: minimal percentage of flops required for
*                    performing matrix-matrix multiplications instead
*                    of pipelined orthogonal transformations;
*               =84: width of block column slabs for row-wise
*                    application of pipelined orthogonal
*                    transformations in their factorized form;
*               =85: the maximum number of eigenvalues moved together
*                    over a process border;
...
*     ..
*     .. External Functions ..
      INTEGER            IEEECK, PIPARMQ, PIPARMQZ, ICEIL
      EXTERNAL           IEEECK, PIPARMQ, PIPARMQZ, ICEIL
*     ..
*     .. Executable Statements ..
*
      IF( ISPEC .GE. 86) GO TO 990
      IF( ISPEC .GE. 50 .AND. ISPEC .LE. 56) GOTO 165

      IF (ISPEC .EQ. 80) GOTO 300
      IF (ISPEC .EQ. 81) GOTO 310
      IF (ISPEC .EQ. 82) GOTO 320
      IF (ISPEC .EQ. 83) GOTO 330
      IF (ISPEC .EQ. 84) GOTO 340
      IF (ISPEC .EQ. 85) GOTO 350
...
*
  160 CONTINUE
*
*     12 <= ISPEC <= 16: PxHSEQR or one of its subroutines.
*
      PILAENVX = PIPARMQ( ICTXT, ISPEC, NAME, OPTS, N1, N2, N3, N4 )
      RETURN
*
*     50 <= ISPEC <= 56: PxHGEQZ or one of its subroutines.
*
  165 CONTINUE
      PILAENVX = PIPARMQ (ICTXT, ISPEC, NAME, OPTS, N1, N2, N3, N4 )
      RETURN
...
*
 ...
 300  CONTINUE
*
*     ISPEC = 80: maximum number of independent computational windows
*
      CALL BLACS_GRIDINFO( ICTXT, NPROW, NPCOL, MYROW, MYCOL )
      PILAENVX = MIN( ICEIL(N1,N2), MIN( NPROW, NPCOL) )
      RETURN
 ...
 310  CONTINUE
*
*     ISPEC = 81: number of eigenvalues in each window
*
      PILAENVX = N2 / 2
      RETURN
*
 320  CONTINUE
*
*     ISPEC = 82: computational window size
*
      PILAENVX = N2
      RETURN
*
 330  CONTINUE
*
*     ISPEC = 83: minimal percentage of flops required for
*     performing matrix-matrix multiplications instead of
*     pipelined orthogonal transformations
*
*
...
*
 340  CONTINUE
*
*     ISPEC = 84: width of block column slabs for row-wise
*     application of pipelined orthogonal transformations in
*     their factorized form
*
*
...
*
 350  CONTINUE
*     
*     ISPEC = 85: maximum number of eigenvalues to bring over
*                 the block border
*
*
      PILAENVX = N2/2
