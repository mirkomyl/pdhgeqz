...
*                                                                     * 
***********************************************************************
      SUBROUTINE PDHGEQZ8( ILSCHR, ILQ, ILZ, IFIRST, ILAST, ILO, IHI,
     $   H, DESCH, T, DESCT, ALPHAR, ALPHAI, BETA, Q, DESCQ, Z, DESCZ, 
     $   ILOQ, IHIQ, ILOZ, IHIZ, GMAXWIN, ND, SELECT, DWORK, LWORK, 
     $   INFO )
      IMPLICIT NONE
*     ..
*     .. Scalar Arguments ..
...
*     ..
     ...
     $   UPCNT, DWCNT, NUM0ZEL, I, GMOVECNT, WINSZ, NPROCS, IAM
*     ..  
*     ..  Local Arrays
*     ..
      INTEGER             UPOS( GMAXWIN ), VPOS( GMAXWIN ), 
     $   ENDPOS( GMAXWIN ), BLKSZ( GMAXWIN ), MOVECNTS( GMAXWIN )
...
*     
      ...
      FIRSTBLK = .TRUE.
      LASTBLK = .FALSE.
      DO WHILE ( J .GT. MYIFIRST .AND. NUMWIN .LT. MAXWIN)
         NUMWIN = NUMWIN + 1
*        
...
*        
         DO SWIN = 1, 2
            DO CURRWIN = SWIN, NUMWIN, 2          
               ...
               RSRC1 = INDXG2P( JJ - BSIZE + 1, NB, 
     $            0, DESCH( RSRC_ ), NPROW)
               CSRC1 = INDXG2P( JJ - BSIZE + 1, NB, 
     $            0, DESCH( CSRC_ ), NPCOL)               
               CSRC4 = INDXG2P( JJ, NB, 
     $            0, DESCH( RSRC_ ), NPROW)
               RSRC4 = INDXG2P( JJ, NB, 
...
*                 not counting the first or last element since those potential zeros in T
*                 cant be moved right now. 
*                 
                  NUM0ZEL = 0
                  
                  DO I = JJ - 1, JJ - BSIZE + 2, -1 
...
*                 
                  MOVECNT = 0                              
                  STARTJ = JJ .EQ. MYILAST
                  CALL DHGEQZ7( 'D', BSIZE, BSIZE, 
     $               TMPH,  MAXWSIZE, TMPT, MAXWSIZE, 
     $               UV( UPOS( CURRWIN ), 1 ), 
     $               UV( VPOS( CURRWIN ), 1 ),
     $               LDUV, SELECT( JJ - BSIZE + 1 ), 
     $               STARTJ, MOVECNT )
*                 
...
*        
*        Skip this part if serial
         IF ( NPROCS .EQ. 0 ) GOTO 250
...
*              
               ...
               CALL PDHGEQZ6('R', ILSCHR, ILO, ILZ, 
     $            H, DESCH, T, DESCT, Q, DESCQ, Z, DESCZ,
     $            UV( UPOS( CURRWIN ), 1 ), 
     $            UV( VPOS( CURRWIN ), 1 ), LDUV, 
     $            1, JJ - BSIZE + 1, JJ, IHI - ILO + 1, 
...
*              
               ...
               CALL PDHGEQZ6('L', ILSCHR, ILQ, ILZ, 
     $            H, DESCH, T, DESCT, Q, DESCQ, Z, DESCZ, 
     $            UV( UPOS( CURRWIN ), 1 ), 
     $            UV( VPOS( CURRWIN ), 1 ), LDUV, 
     $            1, JJ - BSIZE + 1, JJ, IHI - ILO + 1, 
...
*        If MYILAST has changed, recalculate maximum number of windows
*        

         IF ( ENDPOS( 1 ) .EQ. MYILAST .AND. MOVECNTS( 1 ) .GT. 0 ) THEN
*           
*           Set the BETA to zero to indicate an deflated infinite eigenvalue
*           Also set ALPHAR and ALPHAI
            MOVECNT = MOVECNTS( 1 )
            DO I = MYILAST, MYILAST - MOVECNT + 1, -1
               BETA( I ) = ZERO
               ALPHAI ( I ) = ZERO
     ...
     $            H, I,  I, DESCH)
            ...
            MYILAST = MYILAST - MOVECNT
*           
*           Check if done                      
*           
            IF ( MYILAST .LT. MYIFIRST ) GOTO 500
...
*           
            ...
            NUMWIN = 0
            DO WHILE ( J .GE. MYIFIRST .AND. NUMWIN .LT. MAXWIN )
               NUMWIN = NUMWIN + 1
*              
*              Number of rows and column in current block is BSIZE          
*              
...
*              
               J = J - BSIZE + 1
               J = MAX( J - NB / 2, MYIFIRST )
*              
*              Try to add NB to new endpos I                  
*              
               I = MIN( J + NB - 1, MYILAST )
               BSIZE = I - J + 1
               J = I
            END IF
...
*              
*              No crossborder will be performed for last block
*              and therefore we skip that block 
*              
               J = J - BSIZE
*              
