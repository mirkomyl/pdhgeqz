...
*
      ...
      SUBROUTINE PDLAPRNT2( M, N, A, IA, JA, DESCA, IRPRNT, ICPRNT,
     $                     CMATNM, NOUT, WORK, ITER1, ITER2)
*
*  -- ScaLAPACK tools routine (version 1.5) --
*     University of Tennessee, Knoxville, Oak Ridge National Laboratory,
*     and University of California, Berkeley.
...
*
      ...
      DO 60 H = 0, JB-1
         ...
         IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
            IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
               DO 10 K = 0, IB-1
c                  WRITE( NOUT, FMT = 9999 )
c     $                   CMATNM, IA+K, JA+H, A( II+K+(JJ+H-1)*LDA )                  
                  IF (ITER1.GT.9) THEN
                      WRITE( NOUT, FMT = 9998 )
     $                    CMATNM, ITER1, ITER2, IA+K, JA+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                  ELSE
                      WRITE( NOUT, FMT = 9999 )
     $                    CMATNM, 0, ITER1, ITER2, IA+K, JA+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                  END IF                  
   10          CONTINUE
            END IF
     ...
     $                       ICURROW, ICURCOL )
               DO 20 K = 1, IB
c                  WRITE( NOUT, FMT = 9999 )
c     $                   CMATNM, IA+K-1, JA+H, WORK( K )

                  IF (ITER1.GT.9) THEN
                      WRITE( NOUT, FMT = 9998 )
     $                    CMATNM, ITER1, ITER2, IA+K-1, JA+H, 
     $                    WORK( K )
                  ELSE
                      WRITE( NOUT, FMT = 9999 )
     $                    CMATNM, 0, ITER1, ITER2,  IA+K-1, JA+H, 
     $                        WORK( K )            
                  END IF                  
   20          CONTINUE
            END IF
         ...
         DO 50 I = IN+1, IA+M-1, DESCA( MB_ )
            ...
            IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
               IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  DO 30 K = 0, IB-1
c                     WRITE( NOUT, FMT = 9999 )
c     $                      CMATNM, I+K, JA+H, A( II+K+(JJ+H-1)*LDA )

                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,  I+K, JA+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2,  I+K, JA+H, 
     $                        A( II+K+(JJ+H-1)*LDA )  
                      END IF
   30             CONTINUE
               END IF
     ...
     $                          ICURROW, ICURCOL )
                  DO 40 K = 1, IB
c                     WRITE( NOUT, FMT = 9999 )
c     $                      CMATNM, I+K-1, JA+H, WORK( K )
                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,  I+K-1, JA+H,
     $                        WORK( K )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2,  I+K-1, JA+H, 
     $                        WORK( K )  
                      END IF                     
   40             CONTINUE
               END IF
         ...
         DO 120 H = 0, JB-1
            ...
            IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
               IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                  DO 70 K = 0, IB-1
c                     WRITE( NOUT, FMT = 9999 )
c     $                      CMATNM, IA+K, J+H, A( II+K+(JJ+H-1)*LDA )
                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,  IA+K, J+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2,  IA+K, J+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                      END IF                     
   70             CONTINUE
               END IF
     ...
     $                          ICURROW, ICURCOL )
                  DO 80 K = 1, IB
c                     WRITE( NOUT, FMT = 9999 )
c     $                      CMATNM, IA+K-1, J+H, WORK( K )
                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,  IA+K-1, J+H,
     $                         WORK( K )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2,  IA+K-1, J+H,
     $                        WORK( K )
                      END IF                       
   80             CONTINUE
               END IF
            ...
            DO 110 I = IN+1, IA+M-1, DESCA( MB_ )
               ...
               IF( ICURROW.EQ.IRPRNT .AND. ICURCOL.EQ.ICPRNT ) THEN
                  IF( MYROW.EQ.IRPRNT .AND. MYCOL.EQ.ICPRNT ) THEN
                     DO 90 K = 0, IB-1
c                        WRITE( NOUT, FMT = 9999 )
c     $                         CMATNM, I+K, J+H, A( II+K+(JJ+H-1)*LDA )
                        
                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,  I+K, J+H, 
     $                        A( II+K+(JJ+H-1)*LDA )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2,  I+K, J+H,
     $                        A( II+K+(JJ+H-1)*LDA )
                      END IF                           
   90                CONTINUE
                  END IF
     ...
     $                             ICURROW, ICURCOL )
                     DO 100 K = 1, IB
c                        WRITE( NOUT, FMT = 9999 )
c     $                         CMATNM, I+K-1, J+H, WORK( K )
                      IF (ITER1.GT.9) THEN
                          WRITE( NOUT, FMT = 9998 )
     $                        CMATNM, ITER1, ITER2,   I+K-1, J+H, 
     $                        WORK( K )
                      ELSE
                          WRITE( NOUT, FMT = 9999 )
     $                        CMATNM, 0, ITER1, ITER2, I+K-1, J+H, 
     $                        WORK( K )
                      END IF                          
  100                CONTINUE
                  END IF
...
*
  130 CONTINUE
*
 9999 FORMAT(A,I1,I1,I1,'(',I6,',',I6,')=',D30.18,';')
 9998 FORMAT(A,I2,I1,'(',I6,',',I6,')=',D30.18,';')       
