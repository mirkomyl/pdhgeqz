...
*     May 1, 1997
*     
*     Modfied Sept 2015, Bjorn Adlerborn, Umea Univeristy
*     Instead of computing norm of A(L:I, L:I) several times, this info 
*     is saved after first comp. and reused later if needed.
*
*     .. Scalar Arguments ..
      INTEGER            I, K, L, LWORK
...
*     .. Local Scalars ..
     ...
     $                   MYCOL, MYROW, NPCOL, NPROW, NUM, RIGHT, UP
      DOUBLE PRECISION   H10, H11, H22, TST1, ULP, SAVEDNORM
      LOGICAL            HAVENORM
*     ..
*     .. External Functions ..
      INTEGER            ILCM, NUMROC
      DOUBLE PRECISION   PDLAMCH, MPI_WTIME
      EXTERNAL           ILCM, NUMROC, PDLAMCH, MPI_WTIME
*     ..
...
*              No need to calculate this more than once, check if have a previous calculation
               IF ( .NOT. HAVENORM ) THEN
                  CALL INFOG1L( L, HBL, NPROW, MYROW, IAFIRST, ITMP1, 
     $               III )
                  IROW2 = NUMROC( I, HBL, MYROW, IAFIRST, NPROW )
                  CALL INFOG1L( L, HBL, NPCOL, MYCOL, JAFIRST, ITMP2, 
     $               III )
 ...
 30               CONTINUE
               ...
               END IF
            END IF
            IF( ABS( H10 ).LE.MAX( ULP*TST1, SMLNUM ) ) THEN
               GO TO 50
            END IF
            IROW1 = IROW1 - 1
            ICOL1 = ICOL1 - 1
