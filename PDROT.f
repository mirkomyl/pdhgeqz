...
*     .. Local Scalars ..
     ...
     $   MB, NB, XYROWS, XYCOLS, RSRC1, RSRC2, CSRC1, CSRC2, ICOFFXY, 
     $   IROFFXY, MNWRK, LLDX, LLDY, INDX, JXX, XLOC1, XLOC2, RSRC, 
     $   CSRC, YLOC1, YLOC2, JYY, IXX, IYY
      INTEGER            XYROWSP, XYCOLSP, CCNT, RCNT, BUFFSZ
*     ..
*     .. External Functions ..
...
*     
     ...
     $      JX.NE.JY ) THEN
         INFO = -9
         write(*,*)'PDROT err', INCX, INCY, DESCX(M_), DESCY(M_), JX, JY
      END IF
*     
...
*     
     ...
     $         1, CS, SN )
      ...
      END IF
*     
*     Possible here to use a smaller buffert. Now set to N, but set to some 
*     other value for update and communication of smaller messages.
      BUFFSZ = N
*     
...
*                    Split data into chunks 
                     DO 20 XYCOLSP = 1, XYCOLS, BUFFSZ 
                        CCNT = MIN( XYCOLS - XYCOLSP + 1, BUFFSZ ) 
                        CALL DGESD2D( ICTXT, 1, CCNT,
     $                     X( ( XLOC2 - 1 ) * LLDX + XLOC1 ), LLDX, 
     $                     RSRC2, CSRC )
                        CALL DGERV2D( ICTXT, 1, CCNT, WORK, 1,
     $                     RSRC2, CSRC )
                        CALL DROT( CCNT, 
     $                     X( ( XLOC2 - 1 ) * LLDX + XLOC1 ),
     $                     LLDX, WORK, 1, CS, SN )
                        XLOC2 = XLOC2 + CCNT                     
 20                  CONTINUE
                  ELSE
...
*                 Split data into chunks
     ...
     $                  RSRC1, CSRC )
                     CALL DGERV2D( ICTXT, 1, CCNT, WORK, 1, 
     $                  RSRC1, CSRC )
                     CALL DROT( CCNT, WORK, 1, 
     $                  Y( ( YLOC2 - 1 ) * LLDY + YLOC1 ),
     $                  LLDY, CS, SN )
                     YLOC2 = YLOC2 + CCNT
 30               CONTINUE
               END IF
 ...
 10      CONTINUE
     ...
     $            MYCOL, XLOC1, XLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
                  IF( CSRC1.NE.CSRC2 ) THEN
*                    Split data into chunks
                     DO 60 XYROWSP = 1, XYROWS, BUFFSZ
                        RCNT = MIN(XYROWS - XYROWSP + 1, BUFFSZ )
                        CALL DGESD2D( ICTXT, RCNT, 1,
     $                     X( ( XLOC2 - 1 ) * LLDX + XLOC1 ), 
     $                     LLDX, RSRC, CSRC2 )
                        CALL DGERV2D( ICTXT, RCNT, 1, WORK, RCNT,
     $                     RSRC, CSRC2 )
                        CALL DROT( RCNT, 
     $                     X( ( XLOC2 - 1 ) *LLDX + XLOC1 ),
     $                     1, WORK, 1, CS, SN )
 ...
 60                  CONTINUE
     ...
     $            MYCOL, YLOC1, YLOC2, RSRC, CSRC )
               IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC ) THEN
*                 Pplit data into chunks
                  DO 80 XYROWSP = 1, XYROWS, BUFFSZ
                     RCNT = MIN(XYROWS - XYROWSP + 1, BUFFSZ ) 
                     
                     CALL DGESD2D( ICTXT, RCNT, 1,
     $                  Y( ( YLOC2 - 1 ) * LLDY + YLOC1 ), 
     $                  LLDY, RSRC, CSRC1 )
                     CALL DGERV2D( ICTXT, RCNT, 1, WORK, XYROWS,
     $                  RSRC, CSRC1 )
                     CALL DROT( RCNT, WORK, 1, 
     $                  Y( ( YLOC2 - 1) * LLDY + YLOC1 ),
     $                  1, CS, SN )
                     YLOC1 = YLOC1 + RCNT
 80               CONTINUE
